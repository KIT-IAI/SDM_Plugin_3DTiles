...
def json2tileset(thisjson,glbfiles):
    ...
    tileset_dict = json.loads(json_str.replace('/v1/3dtiles/datasets/CgIYAQ/files/','./'))
    # 示例 glb 列表
...
def download_tileset(lon,lat,radius,api_key,output_path,clip_checkbox ,thread_count=30):
    ...
    def download_tile(tile, index):
        data = tile.data
        files.append(f"{tile.basename}.glb")
        #write filename as index to prevent too long filenames
        with open(outdir / Path(f"{index}.glb"), "wb") as f:
            f.write(data)
        return index, True
    ...
    glbfiles_long = ['/v1/3dtiles/datasets/CgIYAQ/files/'+glbfile for glbfile in glbfiles] 
    ...
    import json
    clip_checkboxBool = False
    if clip_checkbox == "1":
        clip_checkboxBool = True
    tileset = {'root': tileset,
    'asset':{'generatetool':'transbigdata',
            'version':'1.0', 'clipToBoundingBox': clip_checkboxBool},
    }
    ...
    #replace long uri paths with index
    with open(f'{output_path}/tileset.json',"r+") as f:
        d = f.read()
        for i, file in enumerate(files):
            d = d.replace(file, str(i) + ".glb")
